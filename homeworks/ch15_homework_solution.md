...
- 공명 진동수가 $\omega_0$에서 약간 아래로 이동한다:

$$\omega_\text{res} = \sqrt{\omega_0^2 - \dfrac{b^2}{2m^2}}$$

$\boxed{\text{감쇠가 클수록 공명 곡선이 낮고 넓어진다.}}$

**(d)** 타코마 내로우즈 다리는 바람이 일정한 속도로 불 때, 와류(vortex)가 다리의 비틀림 진동에 주기적인 힘을 가했고 이 진동수가 다리 고유 진동수에 가까워 큰 진폭을 만들어 결국 붕괴에 이르렀다 ($\boxed{\text{공명에 가까운 현상으로 설명한다.}}$).

> 더 정확한 현대 해석: 단순 공명보다 **공력탄성 플러터(aeroelastic flutter)** 가 주된 메커니즘이라는 것이 정설이다 — 공기 흐름과 구조의 상호작용이 자기 강화 진동을 만들었다. 일반물리에서는 공명 예시로 자주 인용되지만 엄밀히는 차이가 있다.
