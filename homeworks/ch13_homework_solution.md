...
**(c)** 두 사람 사이의 만유인력은 지구 중력의 약 $5 \times 10^{-10}$ 배(백분율 $\times 100\%$로 환산하면 $\approx 5\times 10^{-8}\,\%$, 즉 약 $0.00000005\%$)에 불과하다. 이처럼 극도로 작은 힘이므로, 일상생활에서 사람들 사이의 만유인력은 감지할 수 없다. 만유인력이 의미 있는 크기가 되려면 적어도 하나의 질량이 행성 규모여야 한다.
...
크기로 본 중력은 항상 **중심 방향(인력)** 이며, 그 크기는

$$\boxed{|F| = G\frac{m M_\text{ins}}{r^2} = \frac{GmM}{R^3}r \quad (\text{중심 방향})}$$

**(b)** 중심을 원점으로 잡고, 바깥 방향을 양의 $r$로 약속하면 중력은 항상 $-\hat{r}$ 방향이므로 부호 포함:
