...
1. `website/src/simulations/이름.html` 파일 생성 (단일 HTML)
2. `website/src/data/simulations.ts`에 메타데이터 추가 (id, file, title, chapter, description)
   - `description` 필드의 수식은 `$...$` LaTeX 표기 사용 (메인 페이지에서 KaTeX SSR로 렌더링됨)
...
12. **수식 렌더링은 KaTeX 사용**: 시뮬레이션 HTML 내의 모든 수식 텍스트는 KaTeX CDN(`https://cdn.jsdelivr.net/npm/katex@0.16.11/dist/`)으로 렌더링한다. 컨트롤 패널의 수식은 `katex.renderToString()`으로 HTML에 삽입하고, 캔버스 위의 수식은 절대 위치 HTML 오버레이(`.canvas-overlay`)로 렌더한다. 단순 레이블(`x`, `m` 등)은 캔버스 텍스트로 유지한다.
13. **동적 요소 겹침 방지**: 동적으로 위치가 변하는 레이블(힘 벡터, 위치 표시, 그래프 레이블 등)이 서로 겹치지 않도록 거리 검사를 수행한다. 겹칠 경우 수직 오프셋, 합치기, 또는 숨김 처리한다. 중요 레이블에는 반투명 흰 배경 패딩(`drawTextWithBg`)을 적용한다.
14. **에너지 바 차트**: 바가 커져도 제목("에너지(J)")을 가리지 않도록, 제목은 바 영역 위에 고정하고 값 레이블이 제목과 겹칠 경우 바 내부에 흰색으로 표시한다.
