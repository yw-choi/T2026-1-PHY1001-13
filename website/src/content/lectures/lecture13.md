...
$$F_\text{net} = \sqrt{F_{12}^2 + F_{13}^2} = \sqrt{(4.00)^2 + (1.00)^2} \times 10^{-6} = 4.1 \times 10^{-6}\;\text{N}$$

방향:

$\vec{F}_{12}$는 $+y$, $\vec{F}_{13}$은 $-x$ 방향이므로, 알짜 힘의 성분은 $F_x = -1.00 \times 10^{-6}$ N, $F_y = 4.00 \times 10^{-6}$ N (제2사분면).

$$\theta = \tan^{-1}\frac{F_y}{F_x} = \tan^{-1}\frac{4.00}{-1.00} = -76° \quad \text{(계산기 출력)}$$

계산기의 $\tan^{-1}$은 1·4사분면 값을 주므로, 2사분면으로 보정:

$$\theta = -76° + 180° = 104° \quad \text{($+x$ 축에서 반시계 방향)}$$

**연속 분포:** 확장된 물체의 경우, 미소 질량 $dm$에 의한 미소 힘 $d\vec{F}$를 적분한다:

$$\vec{F}_1 = \int d\vec{F}$$

균일한 구 또는 구형 껍질이면 구각 정리를 이용하여 질량이 중심에 모인 점입자처럼 취급할 수 있다.
...
ISS 고도에서도 $a_g \approx 8.7\;\text{m/s}^2$ — 지상의 약 89%! 우주 비행사가 무중력을 느끼는 이유는 중력이 없어서가 아니라, 우주 정거장과 함께 **자유 낙하** 하고 있기 때문이다.
...
> 서울에서 브라질까지 터널을 뚫으면, 약 42분 만에 도착할 수 있다!
...
이렇게 작은 영역에 이만큼의 질량이 있으므로 → **초대질량 블랙홀(supermassive black hole)** 이다. 2020년, 라인하르트 겐첼과 안드레아 게즈는 이 관측으로 노벨 물리학상을 수상했다.
